...
- [ ] **T-A1-01**: 구독 결제 UI 연결 — `SubscriptionModal` → `/api/subscription/create` 연동
- [ ] **T-A1-02**: 구독 가격 티어 UI — ₩9,900/월 + ₩79,000/년 (일 216원 프레이밍)
- [ ] **T-A1-03**: 구독 상태 확인 UI — `/my` 페이지에 구독 뱃지 + 관리 링크
- [ ] **T-A1-04**: 구독자 전용 혜택 게이팅 — 무제한 Oracle Chat, 프리미엄 테마

### Sprint A-2: 이메일 드립 시퀀스 (전환율 증폭)
- [ ] **T-A2-01**: D+2 이메일 — "리딩 어떠셨어요?" + 20% 할인 코드
- [ ] **T-A2-02**: D+5 이메일 — 천체 이벤트 + Phase 4 연결 CTA
- [ ] **T-A2-03**: D+7 이메일 — "분석 보관 처리 예정" 손실회피 트리거
