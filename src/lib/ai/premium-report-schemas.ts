...
export const PremiumPhase3Schema = z.object({
  ...
  numerology: z.object({
    ...
    personal_year: z.object({
      year: z.number().int().optional(),
      number: z.number().int().min(1).max(9),
      keyword: nonEmptyString,
      theme: nonEmptyString,
      action_tag: z.enum(['PUSH', 'PIVOT', 'HARVEST', 'DEFEND']).optional(),
    }).strict().optional(),
    decision_strategy: z.object({
      energy_type: nonEmptyString,
      strategy: nonEmptyString,
      authority: nonEmptyString,
      saju_mapping: nonEmptyString.optional(),
    }).strict().optional(),
...
export const PremiumPhase8Schema = z.object({
  ...
  final_verdict: z.object({
    ...
    tarot_insight: z.string().optional().default(''),
