...
<SAFETY_RETRY>
Rewrite regulated guidance as decision-support boundaries only. For wellness, use observation routines and professional consultation boundaries. For visa, immigration, legal, tax, and financial-risk cases, use document checks, deadline mapping, questions for qualified professionals, risk buffers, and consultation triggers. Do not name clinical actions, drug-management actions, operation scheduling, visa/status actions, stay/return commands, return-logistics commands, flight booking, application filing/submission/receipt commands, stop/switch/choose commands for immigration paths, signature/filing/lawsuit commands, named market instruments, speculative digital assets, borrowed-risk tactics, concentration bets, exposure sizing, or loss-recovery instructions. Avoid conditional commands like "if no document by X, return/book/file/stop/switch"; write review thresholds and scenario options instead.
Safe replacement examples: "전문가에게 확인할 질문은 무엇인가", "문서와 마감 체크리스트를 점검한다", "A/B 시나리오의 비용과 리스크를 비교한다", "특정 선택을 확정하지 말고 상담 후 재검토한다". Forbidden examples: "서류를 접수해야 한다", "귀국 준비로 전환하라", "비행기 표를 예매하라", "체류/귀국을 확정하라", "잔류와 귀국 중 선택해야 한다", "비자 스폰서십 확보에만 집중하라", "체류 연장 시도를 중단하라", "귀국 절차를 밟기 시작하라".
For the first summary sentence in regulated topics, use only this shape: "첫 행동은 전문가에게 물어볼 질문 목록과 문서/비용 비교표를 작성하는 것이고, 결정 경계는 전문가 검토 전 특정 선택을 확정하지 않는 재검토 기준입니다." Do not put stay/return/apply/extend/file/submit/book/stop/switch words in the first action or boundary.
For astrology/tarot/life-area section endings in regulated topics, use this safe ending shape: "따라서 이 항목의 실천은 문서/질문/비용 비교 점검이며, 전문가 검토 전 특정 선택 확정은 보류하는 재검토 기준으로 둡니다." Do not mention stay/return/apply/extend/file/submit/book/stop/switch in the ending sentence.
</SAFETY_RETRY>` : '';
