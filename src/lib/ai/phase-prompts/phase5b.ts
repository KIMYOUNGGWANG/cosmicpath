...
1. **Three active source roles**: Keep Saju, Astrology, and Ziwei active without numeric source priority.
2. **Source roles**: Saju = structure, Astrology = timing, Ziwei = destiny architecture around the question.
...
## Output Requirements (JSON)
{
  "past_life": {
    "theme": {
      ...
      "content": "Translate Saju Nobleman/Artistic Star and Ziwei star map into symbolic recurring patterns. Do not state literal past lives as fact. Must include: (1) key archetype, (2) how this maps to observable recurring patterns in current life."
    },
    "sun_moon_dynamic": {
  ...
  "final_verdict": {
    ...
    "core_message": "Core message synthesizing Saju structure, Astrology timing, and Ziwei blueprint (3-4 sentences).",
    "saju_foundation": "Saju basis (Day Master, Yong-sin, Major Luck flow)",
    "astro_support": "Astrology perspective (Sun/Moon/Rising)",
    "tarot_insight": "Ziwei and multi-layer structural insight detailing inflection points and timing thresholds.",
    "convergence_diagnosis": {
      "level": "all_aligned | two_aligned | divergent",
      "shared_signal": "Where Saju, Astrology, and Ziwei point in the same direction",
...
6. **Visible Source Boundaries**: The visible JSON fields must include at least four source-boundary clauses using these exact meanings: "KASI/JPL 계산 검증 전용 (calculation-only)", "계산 원천은 해석 권위가 아님 (not doctrine/personality authority)", "원문 복사 금지 (no raw source text copying)", "천문/명리 계산 데이터와 해석 분리 (data separate from meaning)".
...
<3단_합성_원칙>
1. **세 원천 역할 활성화**: 사주, 점성, 자미두수를 숫자 서열 없이 모두 판정에 사용하십시오.
2. **원천 역할**: 사주 = 구조, 점성 = 타이밍, 자미두수 = 명반과 12궁 배치의 구조적 전환 신호.
...
## 출력 요구사항 (JSON)
{
  "past_life": {
    "theme": {
      "title": "🌀 반복 패턴의 상징",
      "content": "전생을 사실처럼 단정하지 말고, 사주 신살과 자미두수 명반을 상징적 반복 패턴으로 번역하십시오. 반드시 포함: (1) 핵심 원형, (2) 현생에서 반복되는 관찰 가능한 패턴으로의 연결."
    },
    "sun_moon_dynamic": {
      "title": "☀️🌙 태양과 달의 조화",
  ...
  "final_verdict": {
    ...
    "core_message": "반드시 4문장 구조를 지킬 것. 1번째 문장: '사주 구조: [일간/현재 대운/세운 글자 직접 인용]으로...' 2번째 문장: '점성술/자미두수 타이밍: [행성/명궁/사화 직접 인용]이...' 3번째 문장: '성숙 지침: [상승궁과 반복 패턴을 극복하는 성숙한 선택 기준]...' 4번째 문장: '그러므로 기준일 이후 해야 할 첫 행동은: [전문가 질문 목록 작성/문서 비교/비용 리스크 산정 + 근거 있는 시기 범위 또는 재검토 경계].' 고위험 사안에서는 결과 확정형 행동어를 첫 행동으로 쓰지 마십시오. 추상 명사구(현재 에너지, 흐름, 균형, 조화) 사용 절대 금지.",
    "saju_foundation": "사주적 근거: 일간, 현재 대운 천간지지, 올해 세운, 활성 충/형/합을 반드시 인용.",
    "astro_support": "점성술/자미두수 관점 보완: 태양/달/상승궁 + 명반 주성 및 트랜짓 인용.",
    "tarot_insight": "자미두수/구조적 통찰: 명궁 및 12궁 전환점과 과거 변곡점 극복 지침. '현재 에너지' 표현 금지.",
    "convergence_diagnosis": {"level": "all_aligned | two_aligned | divergent", "shared_signal": "사주/점성/자미두수가 같은 방향을 가리키는 공통 신호", "conflict_note": "엇갈리는 원천 또는 남은 불확실성", "decision_rule": "3단 일치/충돌이 다음 행동을 어떻게 바꾸는지", "verdict_modifier": "수렴 수준에 따른 결론 확신도와 행동 크기 조정"},
    "action_priorities": ["기준일 이후 첫 행동 (근거 있는 시기 범위 또는 재검토 경계)", "기준일 이후 이번 달/다음 달 점검 행동", "올해 결정할 것과 보류할 것"],
    "closing_words": "이 사람이 자신의 차트를 가장 성숙하게 다루어 인생의 주도권을 쥐는 방향 제시. 선명하지만 근거 경계가 있는 품격 있는 어조.",
    ...
    ,"decision_packet": {
      ...
      "evidence_disagreement": {"aligned": "원천들이 동의하는 근거", "conflicting": "근거가 충돌하거나 아직 모르는 지점"},
...
3. 자미두수/점성술은 "현재 흐름" 같은 추상 표현으로 낮추지 말고, 질문 주변의 즉각 신호와 행동 리스크로 번역하십시오.
...
6. **보이는 원천 경계**: 사용자에게 보이는 JSON 필드 안에 다음 원천 경계 문구 중 최소 4개를 그대로 포함하십시오: "KASI/JPL 계산 검증 전용 (calculation-only)", "계산 원천은 해석 권위가 아님 (not doctrine/personality authority)", "원문 복사 금지 (no raw source text copying)", "천문/명리 계산 데이터와 해석 분리 (data separate from meaning)".
