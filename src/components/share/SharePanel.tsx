...
export function SharePanel({
    ...
    const trackShareEvent = useCallback(async (event: string, metadata?: Record<string, unknown>) => {
        try {
            await trackClientGrowthEvent({
                ...
                metadata: {
                    landingVariant,
                    shareSurface: isEn ? 'global_priority_stack' : 'korean_priority_stack',
                    ...metadata,
                },
            });
        } catch (error) {
    ...
    }, [isEn, landingVariant, language, readingId]);
    ...
    const handleKakaoShare = () => {
        ...
        const trimmedDescription = resolvedShareDescription.length > 120
            ? resolvedShareDescription.substring(0, 120) + '...'
            : resolvedShareDescription;
    ...
    const kakaoOption = (
        ...
        >
            <div className="flex h-10 w-10 items-center justify-center rounded-full transition-transform duration-200 group-hover:scale-105" style={{ backgroundColor: '#FEE500' }}>
                <MessageCircle size={20} style={{ color: '#3C1E1E' }} />
            </div>
            <div className="text-left">
                <p className="font-medium" style={{ color: '#ffffff' }}>{isEn ? 'KakaoTalk' : '카카오톡 공유'}</p>
                <p className="text-xs" style={{ color: 'rgba(255, 255, 255, 0.5)' }}>{isEn ? 'Useful if your audience already uses Kakao' : '친구에게 결과 공유'}</p>
            </div>
        </button>
    );
    const threadsOption = (
        <button
            ...
            className="w-full rounded-xl px-4 py-3 text-left transition-[transform,background-color] duration-200 hover:translate-x-1 hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-300/70"
        >
            <div className="flex h-10 w-10 items-center justify-center rounded-full transition-transform duration-200" style={{ backgroundColor: 'rgba(255, 255, 255, 0.12)' }}>
                <AtSign size={20} style={{ color: '#ffffff' }} />
            </div>
            <div className="text-left">
                <p className="font-medium" style={{ color: '#ffffff' }}>{isEn ? 'Share to Threads' : 'Threads 공유'}</p>
                <p className="text-xs" style={{ color: 'rgba(255, 255, 255, 0.5)' }}>{isEn ? 'Post the hook in one tap' : '한 번에 Threads 게시'}</p>
            </div>
        </button>
    );
    const tiktokOption = (
        <button
            ...
            className="w-full rounded-xl px-4 py-3 text-left transition-[transform,background-color] duration-200 hover:translate-x-1 hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-300/70"
        >
            <div className="flex h-10 w-10 items-center justify-center rounded-full transition-transform duration-200" style={{ backgroundColor: 'rgba(244, 63, 94, 0.2)' }}>
                <Music2 size={20} style={{ color: '#f43f5e' }} />
            </div>
            <div className="text-left">
                <p className="font-medium" style={{ color: '#ffffff' }}>{isEn ? 'Copy TikTok Caption' : 'TikTok 문구 복사'}</p>
                <p className="text-xs" style={{ color: 'rgba(255, 255, 255, 0.5)' }}>{isEn ? 'Optimized for short-form sharing' : '바이럴 최적화 템플릿'}</p>
            </div>
        </button>
    );
    const pdfOption = onPrint ? (
        <button
            key="pdf"
            onClick={() => {
                onPrint();
            ...
            className="w-full rounded-xl px-4 py-3 text-left transition-[transform,background-color] duration-200 hover:translate-x-1 hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-300/70"
        >
            <div className="flex h-10 w-10 items-center justify-center rounded-full transition-transform duration-200" style={{ backgroundColor: 'rgba(255, 255, 255, 0.1)' }}>
                <Download size={20} style={{ color: '#ffffff' }} />
            </div>
            <div className="text-left">
                <p className="font-medium" style={{ color: '#ffffff' }}>{isEn ? 'Save as PDF' : 'PDF 소장본 저장'}</p>
                <p className="text-xs" style={{ color: 'rgba(255, 255, 255, 0.5)' }}>{isEn ? 'High-quality booklet format' : '고퀄리티 책자 형태로 저장'}</p>
            </div>
        </button>
    ) : null;
    const copyLinkOption = (
        <button
            ...
            className="w-full rounded-xl px-4 py-3 text-left transition-[transform,background-color] duration-200 hover:translate-x-1 hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-300/70"
        >
            <div className="flex h-10 w-10 items-center justify-center rounded-full transition-transform duration-200" style={{ backgroundColor: 'rgba(59, 130, 246, 0.2)' }}>
                {copied ? <Check size={20} style={{ color: '#22c55e' }} /> : <Link2 size={20} style={{ color: '#3b82f6' }} />}
            </div>
            <div className="text-left">
                <p className="font-medium" style={{ color: '#ffffff' }}>{copied ? (isEn ? 'Copied!' : '복사됨!') : (isEn ? 'Copy Link' : '링크 복사')}</p>
                <p className="text-xs" style={{ color: 'rgba(255, 255, 255, 0.5)' }}>{isEn ? 'Direct result URL for global sharing' : '결과 페이지 URL'}</p>
            </div>
        </button>
    );
    const compatibilityOption = (
        <button
            ...
            className="w-full rounded-xl px-4 py-3 text-left transition-[transform,background-color] duration-200 hover:translate-x-1 hover:bg-white/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-300/70"
        >
            <div className="flex h-10 w-10 items-center justify-center rounded-full transition-transform duration-200" style={{ backgroundColor: 'rgba(236, 72, 153, 0.2)' }}>
                <Users size={20} style={{ color: '#f472b6' }} />
            </div>
            <div className="text-left">
                <p className="font-medium" style={{ color: '#ffffff' }}>{isEn ? 'Check Compatibility' : '궁합 보기'}</p>
                <p className="text-xs" style={{ color: 'rgba(255, 255, 255, 0.5)' }}>{isEn ? 'Share a compatibility invite with your partner' : '상대방과 오행 비교하기'}</p>
            </div>
        </button>
    );
    const shareOptions = isEn
        ? [copyLinkOption, threadsOption, tiktokOption, compatibilityOption, pdfOption, kakaoOption]
        : [kakaoOption, threadsOption, tiktokOption, pdfOption, copyLinkOption, compatibilityOption];
            ...
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        ...
                        {/* Options */}
                        <div className="p-2">{shareOptions}</div>
                    </motion.div>
                )}
            </AnimatePresence>
        </div>
