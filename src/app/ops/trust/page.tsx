    ...
    if (!session?.user?.id) {
        return (
            <OpsAccessState
                ...
                title="로그인이 필요합니다"
                description="Trust Ops 대시보드는 운영용 보호 페이지입니다. 먼저 로그인한 뒤 권한을 확인해주세요."
            />
        );
    }
    ...
    if (session.user.role !== 'ADMIN') {
        return (
            <OpsAccessState
                eyebrow="Trust Command"
                title="관리자 전용 페이지입니다"
                description="현재 계정은 Trust Ops 대시보드 접근 권한이 없습니다. ADMIN 권한 계정으로 로그인해야 이 페이지를 볼 수 있습니다."
                    ...
                    <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
                        <div className="max-w-3xl">
                            ...
                            <h1 className="mt-4 font-[var(--font-outfit)] text-4xl font-semibold tracking-[-0.05em] text-white">
                                Trust & Incident Dashboard
                            </h1>
                            <p className="mt-4 text-sm leading-7 text-white/64">
                                ops alert, webhook warning, follow-up delivery failure를 모아서 보는 운영 패널입니다.
                                신뢰 이슈와 automation 이슈를 한 화면에서 읽고 우선순위를 정할 수 있습니다.
                            </p>
                        </div>
                    </div>
                </div>
